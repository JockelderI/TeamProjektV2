...
tracker_last_watering= "input_number.helper_last_watering"
threshold_humidity = 30
...
def regression(df, gg):
    ...
    try:
        ...
        print(f"Es wurde zuletzt an folgedem Tag gegoßen: {date_last_watering}")
    except:
    ...
    future_timestamps = np.arange(last_recorded_timestamp, last_recorded_timestamp + 180 * 60 * 8 * 200,
                                  60) #set here how far the prediction should go
...
def print_reg_result_to_HA(helper_id, reg_result):
    ...
    print(f"days until next watering: {days_until_next_watering}")
    if days_until_next_watering < 0:
        days_until_next_watering = 0
        print("days until next watering was set to 0")
...
def print_last_watering_to_HA(helper_id, last_watering):
    ...
    if days_since_last_watering < 0:
        ...
        print("days since last watering was set to 0")
...
execute(plant_id, tracker_next_watering, tracker_last_watering, threshold_humidity)
